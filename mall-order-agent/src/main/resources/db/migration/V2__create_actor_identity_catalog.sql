CREATE TABLE IF NOT EXISTS actor_identity (
    ...
    INDEX idx_actor_identity_category_sort (category, sort_order)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS actor_rag_scope (
    actor_user_id VARCHAR(64) NOT NULL,
    scope_type VARCHAR(32) NOT NULL,
    scope_value VARCHAR(100) NOT NULL,
    PRIMARY KEY (actor_user_id, scope_type, scope_value),
    CONSTRAINT fk_actor_scope_identity FOREIGN KEY (actor_user_id)
        REFERENCES actor_identity(actor_user_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS actor_capability (
    actor_user_id VARCHAR(64) NOT NULL,
    capability VARCHAR(64) NOT NULL,
    PRIMARY KEY (actor_user_id, capability),
    CONSTRAINT fk_actor_capability_identity FOREIGN KEY (actor_user_id)
        REFERENCES actor_identity(actor_user_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS actor_suggestion (
    actor_user_id VARCHAR(64) NOT NULL,
    sort_order INT NOT NULL,
    suggestion VARCHAR(255) NOT NULL,
    PRIMARY KEY (actor_user_id, sort_order),
    CONSTRAINT fk_actor_suggestion_identity FOREIGN KEY (actor_user_id)
        REFERENCES actor_identity(actor_user_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS actor_customer_scope (
    ...
    CONSTRAINT fk_actor_customer_scope_identity FOREIGN KEY (sales_actor_user_id)
        REFERENCES actor_identity(actor_user_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
ON DUPLICATE KEY UPDATE
    category = VALUES(category), display_name = VALUES(display_name), job_title = VALUES(job_title),
    department = VALUES(department), description = VALUES(description),
    welcome_message = VALUES(welcome_message), sort_order = VALUES(sort_order), active = TRUE;

INSERT IGNORE INTO actor_rag_scope (actor_user_id, scope_type, scope_value) VALUES
...
INSERT IGNORE INTO actor_capability (actor_user_id, capability) VALUES
...
INSERT IGNORE INTO actor_suggestion (actor_user_id, sort_order, suggestion) VALUES
...
INSERT IGNORE INTO actor_customer_scope (sales_actor_user_id, customer_user_id) VALUES
    ('SALES001', 'USER1001'), ('SALES002', 'USER1002');
